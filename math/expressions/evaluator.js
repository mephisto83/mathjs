...
export default class Evaluator {
    /**
             ...
             **/
    static evaluate(expression, options) {
    ...
    static evalTrig(expression, options) {
    ...
    static evalLog(expression, options) {
        var Factor = MEPH.math.expression.Factor;
        var Evaluator = MEPH.math.expression.Evaluator;
        var x = expression.partOrDefault(Expression['function'].input);
    ...
    static evalSqrt(expression, options) {
        var Factor = MEPH.math.expression.Factor;
        var Evaluator = MEPH.math.expression.Evaluator;
        var x = expression.partOrDefault(Expression['function'].input);
    ...
    static evalSummation(expression) {
        console.log('Need to actually implement summation.')
        return expression.copy();
    }
    static evalAbs(expression, options) {
        var Factor = MEPH.math.expression.Factor;
        var Evaluator = MEPH.math.expression.Evaluator;
        var x = expression.partOrDefault(Expression['function'].input);
        if (Factor.isNumerical(x)) {
    ...
    static evalE(expression, options) {
        var Factor = MEPH.math.expression.Factor;
        var Evaluator = MEPH.math.expression.Evaluator;
        var x = expression.partOrDefault(Expression['function'].input);
        if (Factor.isNumerical(x)) {
    ...
    static evalLn(expression, options) {
    ...
    static evalVariable(expression) {
        var Factor = MEPH.math.expression.Factor;
        return Factor.getNumerical(expression, true);
    }
    static evalPower(expression, options) {
        var Factor = MEPH.math.expression.Factor;
        var Evaluator = MEPH.math.expression.Evaluator;
        var base = expression.partOrDefault(Expression['function'].base);
    ...
    static evalMultiplication(expression, options) {
        var Factor = MEPH.math.expression.Factor;
        var Evaluator = MEPH.math.expression.Evaluator;
     ...
     **/
    static evalDivision(expression, options) {
        var Factor = MEPH.math.expression.Factor;
        var Evaluator = MEPH.math.expression.Evaluator;
     ...
     * @return {MEPH.math.Expression}
     **/
    static evalAddition(expression, options) {
        var Factor = MEPH.math.expression.Factor;
        var Evaluator = MEPH.math.expression.Evaluator;
    ...
    static orderDependentEval(parts, evalFunc, expFunc) {
        var Evaluator = MEPH.math.expression.Evaluator;
        var Factor = MEPH.math.expression.Factor;
        var currentval, index = 0;
     ...
     * @return {MEPH.math.Expression}
     **/
    static evalSubtraction(expression, options) {
        var Evaluator = MEPH.math.expression.Evaluator;
        var Factor = MEPH.math.expression.Factor;
     ...
     * @return {MEPH.math.Expression}
     **/
    static evalDerivative(expression, options) {
        var Evaluator = MEPH.math.expression.Evaluator;
        var Factor = MEPH.math.expression.Factor;
     ...
     * @return {MEPH.math.Expression}
     **/
    static evalIntegral(expression, options) {
        var Evaluator = MEPH.math.expression.Evaluator;
        var Factor = MEPH.math.expression.Factor;
