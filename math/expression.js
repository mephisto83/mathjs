...
import Set from './set';
import Factor from './expressions/factor';
...
export default class Expression {
    ...
    static Dependency = {
        ...
        VariableRelation: (c, x) => {
            var inRespectTo = x && x.val && x.val.part ? x.val.part('variable').val : x.val;
            return c.respects().contains(function (x) { return x === inRespectTo; });
        },
        ChainRule: (Gx, Fx, expression) => {
        ...
        AlreadyChainedRule: (Gx, Fx, expression) => {
        ...
        OrderRequired: (marks, parentMark, expression) => {
     ...
     */
    static GreatestCommomFactor = (expression) => {
     ...
     **/
    static GreatestCommonDenominator = (expression) => {
     ...
     * @return {MEPH.math.Expression}
     ***/
    static Refactor = (expression, factors) => {
        var flattenedExpression = Expression.Flatten(expression.copy(), Expression.type.multiplication);
     ...
     * @return {MEPH.math.Expression}
     **/
    static Flatten = (expression, type, started) => {
     ...
     * @return {MEPH.math.Expression}
     **/
    static FlattenPower = (expression, type, started) => {
        //
     ...
     * @returns {Array}
     **/
    static createAssociativeGroupings = (expression) => {
     ...
     * @return {Array}
     **/
    static convertGrouping = (grouping, type) => {
     ...
     * @return {MEPH.math.Expression}
     **/
    static convertGroup = (grouping, type) => {
        var expression,
            index = 0,
            buffer = [],
            parentFunc;
    ...
    static matchRule = (expression, rule, markRule) => {
     ...
     * @param {String} type
     **/
    static requiresParenthesis = (type) => {
        switch (type) {
            case Expression.type.subtraction:
            case Expression.type.division:
            case Expression.type.multiplication:
    ...
    static summation = (exp, a, b, respectTo) => {
        var expression = new Expression();
     ...
     * @param {Number} power
     **/
    static trigonometric = (type, exp, power) => {
        var expression = new Expression();
        expression.setExp(type);
    ...
    static derivative = (func, dir, u, t) => {
        var expression = new Expression();
    ...
    setProperties(props) {
        ...
        Object.assign(me.properties, props);
    }
    /**
     ...
     */
    dependency(offset, part, ruleFunc, mark) {
        var me = this;
        me.dependencies.push({
            mark: mark,
            offset: offset,
            part: part,
    ...
    dependenciesAreRespected(expression, after) {
        var me = this;
    ...
    dependenciesMarkAreRespected(rule) {
        var me = this;
